...
def latest_post(request, **kwargs):
    """
    Displays last posted article.
    """

    #from django.http import HttpResponse

    latest = Post.objects.published().latest()

    #return HttpResponse(
    #    "%s/%s/%s/%s" % (
    #    latest.publish.year,
    #    latest.publish.strftime('%b').lower(),
    #    latest.publish.day,
    #    latest.slug,
    #    )
    #)
    ...
    return date_based.object_detail(
        request,
        year=latest.publish.year,
        month=latest.publish.strftime('%b').lower(),
        day=latest.publish.day,
        date_field='publish',
        slug=latest.slug,
        queryset=posts,
        **kwargs
